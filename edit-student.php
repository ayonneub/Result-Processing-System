...
$gender=$_POST['gender'];
$status=$_POST['status'];
$sql="update tblstudents set StudentName=:studentname,RollId=:roolid,gender=:gender,StudentEmail=:studentemail,session=:session,year=:year,Gender=:gender,DOB=:dob,Status=:status where StudentId=:stid ";
...
$sql = "SELECT * from tblstudents where tblstudents.StudentId=:stid";
$query = $dbh->prepare($sql);
$query->bindParam(':stid',$stid,PDO::PARAM_STR);
$query->execute();
...
<div class="col-sm-10">
<input type="text" name="session" class="form-control" id="session" value="<?php echo htmlentities($result->session)?>" required="required" autocomplete="off">
</div>
</div>
...
<div class="col-sm-10">
<input type="text" name="year" class="form-control" id="year" value="<?php echo htmlentities($result->year)?>" required="required" autocomplete="off">
</div>
</div>
